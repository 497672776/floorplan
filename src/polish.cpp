...
const string Polish::VERTICAL_CUT   = "|";
const string Polish::HORIZONTAL_CUT = "-";
Polish::Polish()
{
}
bool Polish::isVerticalCut(string s)
{
...
bool Polish::isHorizontalCut(string s)
{
...
bool Polish::isValidCut(string s)
{
...
int Polish::getCutType(string s)
{
  if (isHorizontalCut(s)) {
    return SlicingNode::HORIZONTAL_CUT;
  }
  if (isVerticalCut(s)) {
    return SlicingNode::VERTICAL_CUT;
  } else {
    // TODO: Throw exception of invalid cut type
    return 3;
  }
}
bool Polish::isValidExpression(const vector<string>& experssion)
{
    ...
    if (Polish::isValidCut(temp)) {
      if (!polishStack.empty()) {
        if (!isValidCut(polishStack.top())) {
          ...
          if (!polishStack.empty()) {
            if (!isValidCut(polishStack.top())) {
              // Not performing pop to simulate keeping computed variable in the
              // top again polishStack.pop();
            } else {
...
bool Polish::isNormalizedExpression(const vector<string>& expression)
{
...
void Polish::printExpression(const vector<string>& expression)
{
  for (auto it = expression.begin(); it != expression.end(); ++it) {
    cout << *it;
  }
}
// Returns indexs where operand is present and where operator is present
// <<vector of operands>,<vecotr of operators>>
pair<vector<int>, vector<int>> Polish::getLocations(
...
// Returns pair of starting and ending points of indexes where consecutive
// operators are found between operands
vector<pair<int, int>> Polish::getRepOperators(const vector<string>& expression)
{
...
string Polish::getCompliment(string s)
{
  if (isValidCut(s)) {
    if (isHorizontalCut(s)) {
      return VERTICAL_CUT;
...
// Returns indexes of operands which can be exchanged with next operator for
// valid expression
vector<int> Polish::getSurroundedOperands(const vector<string>& expression)
{
  // Look for operands
  ...
  for (int i = 0; i < expression.size(); i++) {
    ...
    if (!isValidCut(expression[i])) {
      ...
      // if i is not last element in the expression and i+1 is a valid cut and
      // if the stack size is >=3
      if (i < expression.size() - 1 && isValidCut(expression[i + 1])
          && validatorStack.size() >= 3) {
      ...
      if (i < expression.size() - 2) {
        // Next value is an operand and next to next is the same not operator as
        // the current operator
        if (!isValidCut(expression[i + 1])
            && expression[i].compare(expression[i + 2]) != 0) {
          validIndices.push_back(i);
        }
      }
    }
